...
import { customerData, customerStatuses, sortOptions, branches } from './customer-data';
...
const CustomerList = () => {
  ...
  const handleExportCustomers = () => {
    const csvContent = customerData.map(customer => {
      return [
        customer.name,
        customer.email,
        customer.phone,
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            {[
              { label: 'Total Customers', value: customerData.length, icon: FiUsers },
              { label: 'Active Customers', value: customerData.filter(c => c.status === 'active').length, icon: FiUserCheck },
              { label: 'New This Month', value: 12, icon: FiUserPlus },
              { label: 'Total Revenue', value: `₱${customerData.reduce((acc, curr) => acc + curr.totalSpent, 0).toLocaleString()}`, icon: FiDollarSign }
            ].map((stat, index) => (
                ...
                {currentCustomers.map((customer) => (
                  <tr key={customer.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      ...
                      <div className="text-sm text-gray-500">Last: {customer.lastOrder ? new Date(customer.lastOrder).toLocaleDateString() : 'Never'}</div>
                    </td>
                    <td className="px-6 py-4">
